...
- 首页卡片可横向滑动，天气入口可查看青岛未来三天的天气与温度区间
- 内置仅包含界面文字的精简中文字库
...
├── ui_font_16.c        精简中文字库
├── weather_manager.*   和风天气异步请求、缓存与状态管理
...
复制 `main/weather_private.example.h` 为 `main/weather_private.h`，然后填写和风天气
控制台中的专属 API Host、API Key，以及需要查询地点的经纬度。私有配置已加入
`.gitignore`，不会随仓库提交。天气在联网后自动获取，每 30 分钟刷新；失败后每
5 分钟重试。点击首页天气卡片可进入独立的三日预报页面。

固件使用和风天气 Daily Forecast v1 接口，展示三天的白天天气、最低/最高温度，
并在天气页面保留 `QWeather` 数据归因。

## Wi-Fi 与闹钟说明

- 目前支持 2.4 GHz Wi-Fi；网络名称和密码由 ESP-IDF Wi-Fi 驱动保存在 NVS。
- 配网页面会扫描附近网络，开放网络可直接连接，加密网络会弹出屏幕键盘。
- Wi-Fi 连通后会进行 SNTP 自动校时，并为三日天气预报提供网络连接。
